...
class StructSyntheticProvider:
    ...
    def __init__(self, valobj, dict, is_variant=False):
        ...
        if self.type.IsPointerType():
            self.type = self.type.GetPointeeType()
            #print(f"code = {self.valobj.Dereference().GetChildAtIndex(0)}")
...
class ClangEncodedEnumProvider:
    ...
    def update(self):
        ...
        self.variant = self._getCurrentVariant(all_variants)
        self.is_default = self.variant.GetIndexOfChildWithName(
            ClangEncodedEnumProvider.DISCRIMINANT_MEMBER_NAME) == -1

    def _getCurrentVariant(self, all_variants):
        default_variant = None
        discr_value = None
        sel_by_index = True
        for i in range(all_variants.GetNumChildren()):
            variant = all_variants.GetChildAtIndex(i)
            if variant.GetName() == f"$variant$":
                default_variant = variant

            variant_type_discr = self._getDiscriminator(variant)
            discr = variant.GetChildMemberWithName(
                ClangEncodedEnumProvider.DISCRIMINANT_MEMBER_NAME)
            if discr.IsValid():
                discr_value = discr.GetValueAsUnsigned()
                if discr_value & 0x8000000000000000:
                    discr_value = discr_value & 0x0fffffffffffffff
                    sel_by_index = False

                if not sel_by_index and variant.GetName() == f"$variant${discr_value}":
                    return variant
                if sel_by_index and discr_value != 0x0 and variant_type_discr == discr_value:
                    return variant
        return default_variant

    def _getDiscriminator(self, variant):
        n = variant.GetType().GetName()
        if n.startswith("core::option::Option"):
            if n.endswith("$Inner::None$Variant"):
                return 0x0
            else:
                discr = variant.GetChildMemberWithName(
                ClangEncodedEnumProvider.DISCRIMINANT_MEMBER_NAME)
                if discr.IsValid():
                    ...
                    return 0x0
        if n == "journ_tax::pool::PoolBalance::journ_tax::pool::PoolBalance$Inner::Nil$Variant":
            return 0x8000000000000001
        elif n == "journ_tax::pool::PoolBalance::journ_tax::pool::PoolBalance$Inner::Valued$Variant":
            return None
        elif n == "journ_core::valued_amount::Valuation::journ_core::valued_amount::Valuation$Inner::Total$Variant":
            return 0x01
        elif n == "journ_core::valued_amount::Valuation::journ_core::valued_amount::Valuation$Inner::Unit$Variant":
            return 0x00
        elif n == "journ_tax::deal_holding::DealHolding::journ_tax::deal_holding::DealHolding$Inner::Single$Variant":
            return 0
        elif n == "journ_tax::deal_holding::DealHolding::journ_tax::deal_holding::DealHolding$Inner::Group$Variant":
            return 1
        elif n == "journ_tax::deal_holding::DealHolding::journ_tax::deal_holding::DealHolding$Inner::Sequence$Variant":
            return 0x8000000000000000
        elif n == "journ_tax::deal_holding::DealHolding::journ_tax::deal_holding::DealHolding$Inner::Adjusted$Variant":
            return 0x8000000000000001
        elif n == "journ_tax::deal_holding::DealHolding::journ_tax::deal_holding::DealHolding$Inner::Average$Variant":
            return 0x8000000000000002
        elif n == "journ_tax::adjustment::AmountAdjustment::journ_tax::adjustment::AmountAdjustment$Inner::AddAmount$Variant":
            return 0x00
        elif n == "journ_tax::adjustment::AmountAdjustment::journ_tax::adjustment::AmountAdjustment$Inner::ScaleQuantity$Variant":
            return 0x01
        elif n == "journ_tax::adjustment::ConsiderationAdjustment::journ_tax::adjustment::ConsiderationAdjustment$Inner::AddValuedAmount$Variant":
            return 0x00
        elif n == "journ_tax::adjustment::ConsiderationAdjustment::journ_tax::adjustment::ConsiderationAdjustment$Inner::ScaleQuantity$Variant":
            return 0x01
        else:
            print(f"No discr mapping for {n}")
            return None
