...
impl<'h> Pool<'h> {
    ...
    pub fn try_match(
        ...
        let deal_unit = originator.deal().unit();
        let res = match self.holdings.remove(deal_unit) {
            Some(target) => {
                let amount_remaining = originator.amount().quantity() * dec!(-1);
                let mut target_remaining = Some(target);
                let mut originator_remaining = Some(originator);
                let mut events = vec![];
                loop {
                    let bal_before = target_remaining.as_ref().unwrap().total().clone();
                    let match_method = if bal_before.amount().is_negative() {
                        self.methods.1
                    } else {
                        self.methods.0
                    };
                    match target_remaining.take().unwrap().split_max(
                        originator_remaining.as_ref().unwrap().amount().quantity() * dec!(-1),
                        match_method,
                    ) {
                        Ok((target_matched, target_remaining_opt)) => {
                            let (originator_matched, originator_part_remaining) =
                                originator_remaining
                                    .take()
                                    .unwrap()
                                    .split_max(
                                        target_matched.total().amount().quantity() * dec!(-1),
                                    )
                                    .unwrap();
                            originator_remaining = originator_part_remaining;
                            let event = PoolEvent::new(
                                self.name,
                                event_datetime,
                                PoolEventKind::Match(MatchDetails::new(
                                    match_method,
                                    target_matched,
                                    originator_matched.into_inner(),
                                )),
                                bal_before,
                                target_remaining_opt
                                    .as_ref()
                                    .map(|dh| dh.total().clone())
                                    .unwrap_or_else(|| PoolBalance::nil(deal_unit)),
                            );
                            debug!("{}", event);
                            events.push(event);

                            match target_remaining_opt {
                                Some(tr) => target_remaining = Some(tr),
                                None => {
                                    break Ok((events, originator_remaining));
                                }
                            }
                            if originator_remaining.is_none() {
                                break Ok((events, None));
                            }
                        }
                        Err(target) => {
                            self.holdings.insert(deal_unit, target);
                            if events.is_empty() {
                                break Err(originator_remaining.unwrap());
                            }
                            break Ok((events, originator_remaining));
                        }
                    }
                ...
                /*
                if let Some(right) = target_remaining_opt {
                    self.holdings.insert(deal_unit, right);
                }
                Ok((events, originator_remaining_opt))*/
            }
            None => Err(originator),
        };
        if let Err(originator) | Ok((_, Some(originator))) = &res {
            debug!(
                "{} Unable to match deal: {} on pool: {}",
