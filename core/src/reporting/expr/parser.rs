...
use crate::reporting::expr::Expr;
use crate::reporting::expr::aggregation::{AggState, CoSum, Max, Min, Sum, SumIf, Unique};
use crate::reporting::expr::plan::Plan;
...
pub fn parse_grouping<'h>(input: &'h str) -> JournResult<Vec<Expr<'h>>> {
    ...
    let res = match separated_list0(ws(char(',')), ws(column_spec(&agg_functions)))(input) {
        Ok(("", col)) => Ok(col),
        Ok((remaining, _)) => Err(err!("Unexpected input after parsing grouping: '{}'", remaining)),
        Err(NomErr::Error(e)) | Err(NomErr::Failure(e)) => Err(err!(convert_error(input, e))),
        Err(NomErr::Incomplete(_)) => Err(err!("Incomplete input while parsing grouping")),
    };
    if !agg_functions.borrow().is_empty() {
        Err(err!("Aggregation functions are not allowed in --group-by argument"))
    } else {
        res
    }
}
