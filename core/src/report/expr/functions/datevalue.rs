...
pub fn datevalue<'h>(
    ...
    let time_zone_arg = args.get(2).map(|a| a.eval(context)).transpose()?;
    ...
    if args.len() < 2 {
        ...
        JDateTime::parse(df, tz)(text)
        .map(|(_, d)| d)
        .map_err(|e| {
            err!(
                "Function 'datevalue' could not parse date '{}' with date format '{}' and time zone '{}': {}",
                text,
                df,
                tz,
                e
