...
use crate::err;
use crate::report::command::arguments::DateTimeFormatCommand;
use crate::report::command::print_jerror;
use nom::Finish;
...
impl BeginAndEndArguments {
    ...
    pub fn into_cmd(self, datetime_fmt_cmd: &DateTimeFormatCommand) -> BeginAndEndCommand {
        let dtf = datetime_fmt_cmd.datetime_format_or_default();
...
impl BeginAndEndCommand {
    ...
    pub fn until_end_range(&self) -> impl RangeBounds<JDateTime> {
        let end = self.end.map(Bound::Excluded);
