...
impl DateTimeFormatCommand {
    pub fn new(
        df: Option<&'static DateTimeFormat<'static>>,
        tf: Option<&'static DateTimeFormat<'static>>,
        dtf: Option<&'static DateTimeFormat<'static>>,
        tz: Option<Tz>,
    ) -> Self {
        Self { date_format: df, time_format: tf, datetime_format: dtf, timezone: tz }
    }
    ...
    pub fn from_args_or_config(args: &Arguments, config: &Configuration) -> Self {
        ...
        let (df, tf, dtf, tz) = if dtfc.date_format().is_some()
            || dtfc.time_format().is_some()
            || dtfc.datetime_format().is_some()
            || dtfc.timezone().is_some()
        {
            (dtfc.date_format(), dtfc.time_format(), dtfc.datetime_format(), dtfc.timezone())
        } else {
            (
                Some(config.date_format().clone().into_owned()),
                Some(config.time_format().clone().into_owned()),
                Some(config.datetime_format().clone().into_owned()),
                Some(config.timezone()),
            )
        };

        DateTimeFormatCommand {
            date_format: df,
            time_format: tf,
            datetime_format: dtf,
            timezone: tz,
    ...
    pub fn datetime_from_utc(&self, datetime: &NaiveDateTime) -> JDateTime {
        JDateTime::new(
    ...
    pub fn date_format(&self) -> Option<&'static DateTimeFormat<'static>> {
        self.date_format
    }

    pub fn date_format_or_default(&self) -> &'static DateTimeFormat<'static> {
        self.date_format.unwrap_or(*DEFAULT_DATE_FORMAT)
    }

    pub fn time_format(&self) -> Option<&'static DateTimeFormat<'static>> {
        self.time_format
    }

    pub fn time_format_or_default(&self) -> &'static DateTimeFormat<'static> {
        self.time_format.unwrap_or(*DEFAULT_DATE_FORMAT)
    }

    pub fn datetime_format(&self) -> Option<&'static DateTimeFormat<'static>> {
        self.datetime_format
    }

    pub fn datetime_format_or_default(&self) -> &'static DateTimeFormat<'static> {
