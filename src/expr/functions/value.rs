...
pub fn value<'h, 'a>(
    ...
    match context.as_posting_context() {
        Some(context) => {
            ...
            let base_amount_col =
                args.get(1).cloned().unwrap_or(context.eval_identifier("amount").unwrap());
            ...
            let datetime_col =
                args.get(2).cloned().unwrap_or(context.eval_identifier("datetime_mid").unwrap());
            ...
            let value = if &datetime_col == &context.eval_identifier("datetime_mid").unwrap() {
                SystemValuer::from(context.entry()).value(base_amount, quote_unit)?
            } else {
